...
class ApiServiceRedeem {
  ...
  Future<bool> isCustomerEligible(
      String mobileNumber, String token, int flag, context,
      {TextEditingController? controller}) async {
  ...
  Future<List<VerificationByCardNumber>> getVerificationByCardNumber(
    ...
    try {
      ...
      if (e is SocketException) {}
      rethrow; // Rethrow to allow handling at a higher level if needed
    } finally {
      EasyLoading.dismiss();
    }
  }
  ...
  Future<UserVerificationModel> getUserVerificationDataByCard1(
    ...
    print("Reaward from card headers:>>>$headers");
    try {
      EasyLoading.show(
          dismissOnTap: false, maskType: EasyLoadingMaskType.black);
      final response = await http.get(Uri.parse(apiUrl), headers: headers);
